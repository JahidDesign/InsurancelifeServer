  ...
  try {
    ...
    if (!serviceName || !providerName) {
      return res.status(400).json({
        success: false,
        message: "Service name and provider name are required",
      });
    }
    ...
    const collection = await getOurInsurancePoliceCollection();
...
/* ===================== QUOTE ESTIMATOR ===================== */
// ⚠️ placed BEFORE `/:id` to avoid conflict
router.get("/quote", async (req, res) => {
  try {
    let { age, gender, coverageAmount, duration, smoker, page = 1, limit = 10 } =
      req.query;
    ...
    if (!age || !gender || !coverageAmount || !duration || !smoker) {
      return res
        .status(400)
    ...
    // Premium calculation
    let baseRate = 2; // 2% base
    if (age > 40) baseRate += 1;
    if (smoker.toLowerCase() === "yes") baseRate += 1;
    if (gender.toLowerCase() === "male") baseRate += 0.5;
    ...
    res.json({
      ...
      message: "Estimated premium calculated successfully",
    });
  } catch (err) {
...
// CREATE booking (from InsuranceDetails.jsx "Book Quote" button)
router.post("/bookInsurance", async (req, res) => {
  try {
    ...
    console.error("Book insurance error:", err);
    res.status(500).json({ success: false, message: "Internal server error" });
  }
});

// GET all bookings (admin/agent use)
