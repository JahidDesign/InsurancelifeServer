...
const { getBlogPostCollection } = require('../db');

const router = express.Router();

router.get('/', async (req, res) => {
  try {
    const bookings = await getBlogPostCollection().find().toArray();
    res.json(bookings);
  } catch {
...
router.get('/:id', async (req, res) => {
  try {
    const booking = await getBlogPostCollection().findOne({ _id: new ObjectId(req.params.id) });
...
router.post('/', async (req, res) => {
  try {
    const result = await getBlogPostCollection().insertOne(req.body);
...
router.delete('/:id', async (req, res) => {
  try {
    const result = await getBlogPostCollection().deleteOne({ _id: new ObjectId(req.params.id) });
    result.deletedCount
